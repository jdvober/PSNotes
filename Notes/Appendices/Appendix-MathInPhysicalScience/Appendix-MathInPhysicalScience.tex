...
\quote{Units of measurement give standards so that the numbers from our measurements refer to the same thing.} \autocite{units_of_measurement}
...
\begin{outline}[enumerate]
	...
	\2 I usually write a {\huge\textbf{\purple{?}}} instead of a number, because we are going to solve for this.  We don't know what it is yet!
...
{ % Start of a local group
	...
	\setstackgap{L}{0.5cm} % Set the gap between stacked items
	...
	% Text
	\def\RHST{
		\hspace*{4cm} % move the text to a better location (to the right)
	...
	\def\LHST{
		\hspace*{-1cm} % move the text to a better location (to the left)
		\begin{array}{r}% c --> center aligned
...
\begin{tikzpicture}[overlay, remember picture]
	\draw [->] ([xshift=-0.5cm]pic cs:units1_start) -- ([xshift=0.5cm, yshift=0cm]pic cs:units1_end);
\end{tikzpicture}
